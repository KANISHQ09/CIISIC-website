...
const IndustryChallenges = dynamic(() => import('@/views/industry/challenges'));
...
export default function ChallengesPage() {
  ...
  if (role === 'INDUSTRY_SPOC') {
    return <IndustryChallenges />;
  }

  const filteredChallenges = mockChallenges.filter((ch) => {
    const matchesSearch =
      ch.title.toLowerCase().includes(searchTerm.toLowerCase()) || ch.companyName.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    <div className="w-full min-h-screen bg-slate-50 py-20 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto space-y-12 text-left">
        ...
        <div className="space-y-4">
          ...
          <h1 className="text-3xl sm:text-5xl font-extrabold text-[#0F172A] tracking-tight">Industrial Challenges</h1>
          ...
          {filteredChallenges.map((ch) => (
            ...
            >
              <div className="space-y-4">
                ...
                <h4 className="text-base font-bold text-[#0F172A] group-hover:text-blue-900 transition-colors">{ch.title}</h4>
              ...
              <div className="pt-6 border-t border-zinc-200/60 mt-6 flex items-center justify-between">
                ...
                <Link
                  href={`/challenges/${ch.id.toLowerCase()}`}
                  className="inline-flex items-center gap-1.5 text-xs font-bold text-[#0F294A] hover:text-blue-700"
                >
